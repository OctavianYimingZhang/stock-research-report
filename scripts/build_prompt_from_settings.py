...
def build_prompt(settings: dict[str, Any], identifier: str) -> str:
    lines = [
        ...
        f"- report structure: {value(settings, 'report_structure_version', 'canonical_12_section_v1')}",
        ...
        f"- valuation policy: share count blocker {value(settings, 'valuation_policy.block_target_if_share_count_unresolved')}, net debt blocker {value(settings, 'valuation_policy.block_target_if_net_debt_unresolved')}, senior claims blocker {value(settings, 'valuation_policy.block_target_if_senior_claims_unresolved')}",
        f"- short-risk policy: expand threshold {value(settings, 'short_risk_policy.expand_threshold')}",
        f"- technical policy: current OHLCV {value(settings, 'technical_policy.require_current_ohlcv')}, chart date {value(settings, 'technical_policy.require_chart_date')}, adjusted status {value(settings, 'technical_policy.require_adjusted_status')}",
        "- decision scorecard: require action grade and binding cap reason",
        f"- interaction level: {value(settings, 'interaction_level')}",
        "",
        "Apply the Skill evidence rules and the canonical report contract. Treat user hypotheses and outside articles as thesis paths to test, not as evidence.",
    ]
    return "\n".join(lines)
